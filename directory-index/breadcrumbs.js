...
asyncTest('for folder', function() {
  with($context) {
    ...
    $.when( frame.go(URL) ).then(function(_$) {
        ...
        $.substring( bd_href.last().attr('href'), cwd, 'last link leads to previous(virtual) folder' );
        equal( $(bd_href[1]).html(), cwd, 'first crumb is virtual folder' );
...
asyncTest('for suite', function() {
  with($context) {
    ...
    $.when( frame.go(URL) ).then(function(_$) {
        ...
        equal( bd_href.first().html(), "•" );
        equal( bd_href.first().attr('href'), '/', 'first link leads to root' );
        equal( bd_href.last().attr('href'), '/'.concat(cwd, '/', folder_name, '/'), 'last link leads to upper level' );
...
asyncTest('for test', function() {
  with($context) {
    ...
    $.when( frame.go(URL) ).then(function(_$) {
        ...
        equal( $(bd_href[3]).html(), suite_name );
        equal( $(bd_href[3]).attr('href'), '/'.concat(cwd, '/', folder_name, '/', suite_name, '/'), 'second link leads to parent suite' );
